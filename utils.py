...
def get_test_image(paths, height=256, width=256, flag=False):
    ...
    for path in paths:
        ...
        if h > base_size or w > base_size:
            ...
            edge_image = Get_edgeMap(image)
            edge_images.append(edge_image)
            image = np.reshape(image, [1, image.shape[0], image.shape[1]])
            images.append(image)
            edge_images = np.stack(edge_images,axis=0)
            edge_images = torch.from_numpy(edge_images).float()
            images = np.stack(images, axis=0)
            images = torch.from_numpy(images).float()

    # images = np.stack(images, axis=0)
    # images = torch.from_numpy(images).float()
    return edge_images,images, h, w, c
...
def get_train_images_auto(paths, height=256, width=256, flag=False):
    ...
    for path in paths:
        ...
        edge_iamges.append(edge_image)
        images.append(image)


    edge_images = np.stack(edge_iamges,axis=0)
    edge_images = torch.from_numpy(edge_images).float()
    images = np.stack(images, axis=0)
    images = torch.from_numpy(images).float()

    return edge_images,images
